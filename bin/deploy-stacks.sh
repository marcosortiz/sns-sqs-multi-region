...
# sam deploy --stack-name $STACK_NAME --region $PRIMARY_REGION --config-env $PRIMARY_ENV --capabilities CAPABILITY_IAM --no-fail-on-empty-changeset --resolve-s3
...
# sam deploy --stack-name $STACK_NAME --region $SECONDARY_REGION --config-env $SECONDARY_ENV --capabilities CAPABILITY_IAM --no-fail-on-empty-changeset --resolve-s3
